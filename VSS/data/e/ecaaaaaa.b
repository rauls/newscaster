...
static time_t	last_good_download_t = 0;
	...
	{
		downloadsize = DownloadText( NULL, DEFURL_NATIONALNEWS, CFG->cfgLocalBaseDir + CFG->cfgLocalXML_Dir + XMLFN_NATIONALNEWS );
		if( downloadsize < 1 )
			OutDebugs( "ERROR: News Failed (%d) to download %s", downloadsize, DEFURL_NATIONALNEWS );
		else
			totaldownload += downloadsize;
	}
	if( CFG->cfgUseYahooTicker )
	{
		downloadsize = DownloadText( NULL, DEFURL_YAHOOTOPNEWS, CFG->cfgLocalBaseDir + CFG->cfgLocalXML_Dir + XMLFN_YAHOOWORLDNEWS );
		if( downloadsize < 1 )
			OutDebugs( "ERROR: News Failed (%d) to download %s", downloadsize, DEFURL_YAHOOTOPNEWS );
		else
			totaldownload += downloadsize;
	}
	if( CFG->cfgUseBBCTicker )
	{
		downloadsize = DownloadText( NULL, DEFURL_BBCWORLDNEWS, CFG->cfgLocalBaseDir + CFG->cfgLocalXML_Dir + XMLFN_BBCWORLDNEWS );
		if( downloadsize < 1 )
			OutDebugs( "ERROR: News Failed (%d) to download %s", downloadsize, DEFURL_BBCWORLDNEWS );
		else
			totaldownload += downloadsize;
	}
...
long CWidgieXML::DownloadXMLLoop()
{
	...
	downloadsize =
	DownloadSlideShow( CFG->cfgIPandPort, CFG->cfgLocalBaseDir + CFG->cfgLocalXML_Dir + CFG->cfgLocalLoopName );
...
CString CWidgieXML::GetElementParam( CString p_xml, CString p_paramName )
{
	...
	if( p_xml.IsEmpty() == FALSE )
	{
		int pos = m_slideshowElement.Find( p_paramName );
		if( pos >= 1 )
		{
...
//
//
long CWidgieXML::LoginToMDID( CString server, CString remoteFile, CString parameters ) 
{
				...
				while( httpFile->ReadString(buff)==1 && AFXDIALOG->shuttingDown==FALSE )
				{
					...
					if( buff.Find( "<sessiontoken>" ) >= 0 )
					{
						...
						if( !m_sessionToken.IsEmpty() )
							myInternetSession->SetCookie( server, "sessiontoken", m_sessionToken.GetBuffer(0) );
					}
...
//
//
long CWidgieXML::DownloadSlideShow( CString server, CString fileToWrite ) 
{
	long	downloadsize = 0;
	...
	{
		LoginToMDID( server, URL_MDID_LOGIN, "" );
	}
    ...
    if ((noConnection == false) && (success == TRUE))
    {
		...
		else
		{
			...
			if( localSavedFile.Open(tempFile, CFile::modeCreate | CFile::modeWrite) )
			{
				...
				// ------------------  read slideshow from webservice
				while( httpFile->ReadString(buff)==1 && AFXDIALOG->shuttingDown==FALSE )
				{
					...
					if( !resultCode )
					{
						...
						{ 
							OutDebugs( "ERROR: Slideshow %s not found on server", fileToGet );
							downloadsize = 0; 
							break; 
						}
					}
				...
				if( resultCode && downloadsize>0 && dataToGet>0 )
				{
				...
				if( downloadsize != -1 )
				{
					if( !resultCode )			// clear our session token if we failed to download for any reason.
						m_sessionToken.Empty();

					// download incomplete, delete file and return 0
					OutDebugs( "Download failed - %s, %d/%d bytes done, deleting file now", buff.GetBuffer(0), downloadsize, dataToGet );
...
long CWidgieXML::DownloadMissingContent( long *pfilesTotal, long *pfilesDone ) 
{
	...
	{
		LoginToMDID( CFG->cfgIPandPort, URL_MDID_LOGIN, "" );
	}
	...
	*pfilesDone = 0;
	...
	while( newPos = impList->FindIndex(imageCount++) )
	{
		...
		CString localFilename = CFG->cfgLocalBaseDir + CFG->cfgLocalContentDir + nextImpression.m_image_file;
...
BOOL CWidgieXML::IsServerAlive( void )
{
	...
	if( InternetAttemptConnect( 0 ) == ERROR_SUCCESS )
	{
		...
		CString
		server = URLGetPort( CFG->cfgIPandPort, &port );
        ...
        if (noConnection == false)
        {
			...
			else
			{
				...
				if(	localBinFile.Open( tempFile, CFile::modeCreate | CFile::modeWrite) )
				{
					...
					if( downloadsize && (downloadsize == dataToGet) )
					{
						myDeleteFile( fileToWrite );
