...
{
    char* message = XMLString::transcode(qname);// get message
	if( !message ) return;
	CString messageString = message;			// copy the string to local var
	XMLString::release(&message);				// free the message
	...
	switch (XML_State)
	{
		...
		case FOUND_NEWSFLASH :
			...
			{
				XML_CharState = NEWS_TEXT;
				newsSource = NEWS_YAHOO;
				gotText = true;
			}
			if (messageString.Compare("pub_date") == 0)
			{
