		...
		if ( client_socket != INVALID_SOCKET ){
			...
			while ( ret>0 || ret==keepalive ){
				if ( ret>0 ){
					...
					//OutDebugs( "HTTPD%d: recieved request (%d bytes)", lpParam, ret );
...
long MakeHTTPHeader( char *d, char *file, long type )
{
	...
	if ( type <= 0 ){
		...
		if (type == 2)
		{
			...
			ext = strrchr( file, '.' );
			strcat( d, "Connection: close\r\n" );
			strcat( d, "Content-Type: image/jpeg" );	//strcat( d, ext+1 );
		}	
		else 
		if (type == 3)
		{
...
#define	HEADLINE(text)	"<TABLE width=540><TR><TD bgcolor=blue>" text "</TD></TR></TABLE>"
...
static char HTMLindex[] =
{
	...
	"<A HREF=/debug>Debug Messages Monitor</A><BR><br>"
	...
	"<A HREF=/widgie/newsflash.asp>View news.xml</A><BR>"
	"<A HREF=/widgie/manifest.asp>View manifest.xml</A><BR>"
	"<A HREF=/widgie/loopsequence.asp>View loop.xml</A><BR>"
	"<A HREF=/widgie/config.asp>View config.xml</A><BR>"
	"<BR><BR>"
	HEADLINE( "====== Admin Menu ======" )
	"<A HREF='/action.cgi?command=1'>1.  Clear Image Cache</A><BR>"
	"<A HREF='/action.cgi?command=2'>2.  Reload Play List from disk</A><BR>"
	"<A HREF='/action.cgi?command=3'>3.  Download Manifest XML inc Media Content</A><BR>"
	"<A HREF='/action.cgi?command=4'>4.  Download All XML files</A><BR>"
	"<A HREF='/action.cgi?command=5'>5.  Download News XML content now</A><BR>"
	"<A HREF='/action.cgi?command=6'>6.  Download Missing Image/Media Content</A><BR>"
	...
	"<A HREF='/action.cgi?command=19'>19. Launch VNC Serve</A><BR>"
	...
	"<A HREF='/action.cgi?command=24'>24. Quit to Test Precedure</A><BR>"
	"<A HREF='/action.cgi?command=25'>25. Download new runtime exe</A><BR>"
	"<A HREF='/action.cgi?command=25'>26. Exit Admin Menu</A><BR>"
};
...
long HandleHttpURL( long os, char *line )
{
	...
	if ( !strcmpd( "GET", line ) )
	{
		...
		if ( !strcmpd( "/index.html", paramUrl ) || !strcmp( "/", paramUrl ) ){
			...
			CString page = tmp;
			page += HTMLheader;
			CString text = "Newscaster v" PRODUCT_VERSION_STR " System Control : CLIENT ID = " + CFG->cfgID;
			page += HEADLINE( +text+ );
		...
		if ( !strcmpd( "/debug", paramUrl ) )
		{
			...
			CString page = tmp;
			page += HTMLheader;
			CString text = "Debug Status Monitor : IVA ID = " + CFG->cfgID;
			page += HEADLINE( +text+ );
			d = SendStrHTTP( os, page.GetBuffer(0) );
		...
		if ( !strcmpd( "/status", paramUrl ) )
		{
			...
			page += header;
			CString text = "System Status Info : CLIENT ID = " + CFG->cfgID;
			page += HEADLINE( +text+ );
			page += "<PRE>";
		...
		{
			if( ServeFile( os, "Config/news.xml", outline ) )
				d = 1;
		} else
		...
		if ( !strcmpd( XMLFN_LOOPSEQ, paramUrl )  )
		{
			if( ServeFile( os, "Config/loop.xml", outline ) )
				d = 1;
		} else
